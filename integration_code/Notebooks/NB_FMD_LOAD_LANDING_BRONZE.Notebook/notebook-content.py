...
if schema_enabled == True:
    #Set SourceFile and target Location
    source_changes_data_path = f"abfss://{SourceWorkspace}@onelake.dfs.fabric.microsoft.com/{SourceLakehouse}/Files/adls/{SourceFilePath}"
    #source_changes_data_path = f"abfss://{SourceWorkspace}@onelake.dfs.fabric.microsoft.com/{SourceLakehouse}/Files/{SourceFilePath}/{SourceFileName}"
    print(source_changes_data_path)

    #Beware 
    #target_data_path = f"abfss://{TargetWorkspace}@onelake.dfs.fabric.microsoft.com/{TargetLakehouse}/Tables/{DataSourceNamespace}/{TargetSchema}_{TargetName}"
    target_data_path = f"abfss://{TargetWorkspace}@onelake.dfs.fabric.microsoft.com/{TargetLakehouse}/Tables/{TargetSchema}/{TargetName}"
    print(target_data_path)
elif schema_enabled != True:
    #Set SourceFile and target Location
    source_changes_data_path = f"abfss://{SourceWorkspace}@onelake.dfs.fabric.microsoft.com/{SourceLakehouse}/Files/{SourceFilePath}"
    #source_changes_data_path = f"abfss://{SourceWorkspace}@onelake.dfs.fabric.microsoft.com/{SourceLakehouse}/Files/{SourceFilePath}/{SourceFileName}"
    print(source_changes_data_path)
...
dup_count = (
    dfDataChanged.groupBy('HashedPKColumn')
    .count()
...
if dup_count > 0:
    print(f'Removing duplicate rows for {dup_count} duplicated PK(s)')
